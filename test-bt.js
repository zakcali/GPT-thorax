...
async function run() {
	// preview mode calls during March/April 2024
    const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro-latest", generationConfig, safetySettings, systemInstruction,},
     {apiVersion: 'v1beta'});
	// non-preview model may be like this: const model = genAI.getGenerativeModel({ model: "gemini-1.5-pro-latest", generationConfig, safetySettings, systemInstruction,});
    // read prompt
    let prompt;
    try {
